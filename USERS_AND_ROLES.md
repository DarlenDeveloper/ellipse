...
4. 🟡 Shared-integration access — **request/approve + per-user gating done + deployed** (`access.ts`: `requestSharedAccess`/`respondAccessRequest`/`revokeSharedAccess`; `askAgent` filters connections per user). Requests are exact selections: editing replaces, rather than merges, the requested types. Approval makes the existing company connection immediately usable; it does not copy or personally connect the integration. Owners/admins can remove individual grants. Direct agent calls and report sources re-check grants server-side. **Deferred:** the per-user *personal-connection connect pipeline* (employee OAuth-adds their own integration tagged `scope:"personal"`, `owner_uid`); do not describe this as live.
