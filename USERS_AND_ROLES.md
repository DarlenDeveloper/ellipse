...
4. 🟡 Shared-integration access — **request/approve + per-user gating done + deployed** (`access.ts`: `requestSharedAccess`/`respondAccessRequest`/`revokeSharedAccess`; `askAgent` filters connections per user). Requests are exact selections. Approval makes the existing company connection immediately usable. **Personal Google Workspace is implemented** with owner-scoped OAuth state, connection/secret records, mailbox data, sends/replies and personal disconnect. Microsoft 365, Zoho and credential/API personal connectors still require the same owner-scoped adaptation and must be shown as rolling out.
