...
- Employee **Connect** opens an explicit company-vs-personal choice. Personal Google Workspace now uses separate owner-scoped connection and secret records, signed-in-user OAuth state, owned mailbox records, scoped sends/replies and personal-only disconnect. Remaining providers still need migration to the same model.
