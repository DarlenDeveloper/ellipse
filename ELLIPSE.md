...
**Users & roles (live):** owner/admin/employee + `can_approve`; invited employees auto-join their org (`acceptInvite`); member management (invite/role/approve/remove/revoke); **per-connection shared access** (employees explicitly choose company access vs personal connection, request specific company connections, and owner/admin approve or later remove individual grants); every data surface scoped to a member's granted connections (`useAccess`); daily owner-only team-activity digest on the Data page. Personal per-user OAuth credential storage is still deferred and safely blocked in the UI.
